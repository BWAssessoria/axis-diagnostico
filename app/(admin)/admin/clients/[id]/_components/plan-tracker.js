      ...
      <div className="mt-2.5 flex items-center gap-2 border-t border-white/[0.05] pt-2">
        ...
        <div className="ml-auto flex gap-1">
          {KANBAN_ORDER.indexOf(status) > 0 && (
            <button
              onClick={() => moveTo(KANBAN_ORDER[KANBAN_ORDER.indexOf(status) - 1])}
              disabled={isPending}
              title="Mover para coluna anterior"
              className="rounded px-1.5 py-0.5 text-[9px] text-muted-foreground/50 transition-colors hover:bg-white/[0.05] hover:text-muted-foreground disabled:opacity-30"
            >←</button>
          )}
