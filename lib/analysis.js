...
export function analyze(a) {
  ...
  const equipeOp = (a.equipe || '').toLowerCase();
  const equipeOpRoles = [/secretár|recepcion/, /gestor|gerente|coordena/, /financeiro|administra/, /auxiliar|assistente|técnic/].filter(re => re.test(equipeOp)).length;
  const equipeSolo = /sozinha|só eu|só a profissional|solo/.test(equipeOp) || a.equipe?.trim().length === 0;
  if (!equipeSolo && equipeOpRoles >= 2) sc.operacional += 2;
  else if (!equipeSolo && (equipeOpRoles >= 1 || (a.equipe && a.equipe.trim().length > 10))) sc.operacional += 1;
...
export function analyzeICP(a) {
  ...
  // Equipe (10 pts) — keyword-based: detecta papéis reais na descrição
  const equipeLC   = equipe.toLowerCase();
  const temSec     = /secretár|recepcion/.test(equipeLC);
  const temGestor  = /gestor|gerente|coordena/.test(equipeLC);
  const temFin     = /financeiro|administra/.test(equipeLC);
  const temAux     = /auxiliar|assistente|técnic/.test(equipeLC);
  const temSolo    = /sozinha|só eu|só a profissional|solo|apenas eu/.test(equipeLC) || equipe.trim().length === 0;
  const papeis     = [temSec, temGestor, temFin, temAux].filter(Boolean).length;
  if (!temSolo && papeis >= 3)    { score += 10; criterios.push({ok:true,    txt:"Equipe com múltiplos papéis definidos — operação escalável"}); }
  else if (!temSolo && papeis >= 2) { score += 7; criterios.push({ok:true,    txt:"Equipe estruturada com papéis definidos"}); }
  else if (!temSolo && (papeis >= 1 || equipe.trim().length > 8)) { score += 4; criterios.push({ok:"parcial",txt:"Equipe inicial — papéis ainda não totalmente definidos"}); }
