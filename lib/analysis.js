...
export function analyzeICP(a) {
  ...
  // Faturamento (30 pts) — mínimo R$10k para Método Axis
  ...
  else if (fat >= 10000) { score += 8;  criterios.push({ok:"parcial",txt:"Faturamento entre R$10k–20k — elegível para Método Axis"}); }
  else if (fat > 0)      { score += 2;  criterios.push({ok:false,   txt:"Faturamento < R$10k — indicado GPS Gravado (infoproduto)"}); }
  ...
  if (fat >= 10000) {
    ...
    else                   prodDesc = "Estágio inicial do Método Axis. Foco em construir processo comercial, presença digital e protocolo de valor.";
  } else if (fat > 0) {
    produto = "GPS Gravado"; prodCor = "#9C27B0";
    prodDesc = "Faturamento abaixo de R$10k. O GPS Gravado (infoproduto) é o caminho ideal para estruturar a base antes da assessoria.";
  } else {
...
export function getPacRecomendacoes(produto, plano, data) {
  ...
  if (produto === "GPS Gravado") return [
    { tag:"PROTOCOLO",  acao:"Criar protocolo nomeado a partir do procedimento carro-chefe — transformar procedimentos avulsos em programa de transformação com ticket 3x maior." },
    { tag:"COMERCIAL",  acao:"Estruturar processo de resposta ao lead em até 5 minutos. Script de qualificação e agendamento via WhatsApp com cadência de 3 dias." },
    { tag:"PRESENÇA",   acao: tem_gmn ? "Ativar estratégia de captação de avaliações no Google Meu Negócio. Meta: 15 avaliações nos primeiros 30 dias." : "Criar e verificar Google Meu Negócio. Canal de aquisição orgânica gratuita e essencial para estética avançada." },
    { tag:"CONTEÚDO",   acao:"Implementar calendário editorial básico: 3 posts/semana com 4 pilares — educação, prova social, bastidores e CTA." },
    { tag:"TRÁFEGO",    acao:`Primeira campanha Meta Ads com budget de R$800–1.500/mês focada em leads para avaliação. CPL meta: < ${fmtR(Math.round(tick*0.25))}.` },
    { tag:"MÉTRICA",    acao:"KPI principal: custo por avaliação agendada. Monitorar semanalmente e ajustar criativos com base no resultado." },
  ];
...
export function buildDiagnostico(a, produto, plano) {
  ...
  if (produto === "GPS Gravado") {
    meta45     = Math.round(fat * 1.25 / 1000) * 1000;
    meta45desc = "Protocolo nomeado vendido, processo comercial rodando e primeiros leads chegando via tráfego estruturado";
  } else {
    // Método Axis — escopo de planejamento único
    meta90  = Math.round(fat * 1.30 / 1000) * 1000;
    meta180 = Math.round(fat * 1.60 / 1000) * 1000;
    meta90desc  = "Canais estruturados, campanhas otimizadas e processo comercial funcionando com previsibilidade";
    meta180desc = "Crescimento consistente com múltiplos canais ativos, autoridade digital consolidada e base de pacientes fidelizada";
  }
  ...
  if (produto === "GPS Gravado") {
    planoExecucao = [
      {
        fase:"Semanas 1–2", cor:"#9C27B0",
        titulo:"Diagnóstico + Protocolo",
        objetivo:"Criar o protocolo nomeado e estruturar a proposta de valor premium.",
        acoes:[
          "Mapear procedimentos e criar 1–2 nomes de protocolo com narrativa de transformação",
          "Precificar protocolo a 3x o ticket avulso atual e criar âncora de valor",
          "Criar script de apresentação e venda do protocolo — da avaliação ao fechamento",
          "Configurar CRM básico (Kommo) e funil com etapas e responsáveis",
        ]
      },
      {
        fase:"Semanas 3–4", cor:"#7B1FA2",
        titulo:"Presença Digital Base",
        objetivo:"Estruturar os canais digitais mínimos para captação.",
        acoes:[
          "Otimizar perfil Instagram: bio, foto de perfil e destaques organizados",
          "Criar/otimizar Google Meu Negócio com fotos profissionais e categorias corretas",
          "Lançar campanha de captação de avaliações — meta: 10 avaliações em 30 dias",
          "Configurar pixel Meta e criar landing page básica para o protocolo carro-chefe",
        ]
      },
      {
        fase:"Semanas 5–6", cor:"#6A1B9A",
        titulo:"Captação + Tráfego",
        objetivo:"Gerar os primeiros leads qualificados via tráfego pago.",
        acoes:[
          "Lançar campanhas Meta Ads — budget: R$800–1.500/mês — objetivo: leads para avaliação",
          "Implementar follow-up em até 5 minutos + cadência de 3 dias via WhatsApp",
          "Criar calendário editorial básico: 3 posts/semana com pilares definidos",
          "Treinar atendimento para script de conversão lead → avaliação → protocolo",
        ]
      },
      {
        fase:"Semanas 7–8", cor:"#4A148C",
        titulo:"Primeiros Resultados",
        objetivo:"Mensurar resultados e definir próximos passos com clareza.",
        acoes:[
          "Revisar métricas: CPL, taxa de conversão de lead e ticket médio dos novos pacientes",
          "Coletar primeiros depoimentos de pacientes do protocolo para prova social",
          "Ajustar campanhas com base nos criativos de melhor performance",
          "Avaliar continuidade: Método Axis é o próximo passo natural",
        ]
      },
    ];
  } else {
    // Método Axis — plano de entrega contínua
    planoExecucao = [
      {
...
export function buildCMOAnalysis(a, scores, produto, plano) {
  ...
  else if (fat >= 10000) pontos_fortes.push(`Faturamento de ${fmtR(fat)}/mês já supera o mínimo para o Método Axis — há base para crescer com estratégia.`);
  ...
  if (tick > 0 && tick < 4000) {
    alavancas.push({
      ...
      prazo: produto === "GPS Gravado" ? "Semanas 1–2 do GPS" : "Primeiro mês do Método Axis",
    });
  }
  if (!tem_trafego || inv.includes("Menos de 3")) {
    alavancas.push({
      ...
      prazo: produto === "GPS Gravado" ? "Semanas 3–6" : "Meses 1–2 do Método Axis",
    });
  }
  ...
  let visao;
  if (produto === "GPS Gravado") {
    visao = `O GPS Gravado é o produto certo para ${a.nome_clinica||"esta clínica"} neste momento. Não porque falta ambição — mas porque falta fundação. O trabalho é construir os três pilares que nenhuma campanha resolve sem eles: protocolo de valor, processo comercial e presença digital mínima. Com esses três no lugar, o próximo passo tem ROI real.\n\nA maior alavanca imediata é a engenharia de protocolo — transformar "${carro||"o procedimento carro-chefe"}" de uma venda avulsa em um protocolo de transformação a 3x o ticket atual. Isso não exige novos pacientes nem mais tráfego. Exige posicionamento. E posicionamento é o que muda a matemática do negócio.`;
  } else {
    // Método Axis
